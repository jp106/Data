  ...
  navbarPage(title = "Explorer",
             tabPanel(title = "Data Explorer",
                      fluidRow(
                        ...
                        column(3,selectInput(inputId = "selinput1",label = "Select a State",choices = c(state_codes$StateName)))
                      ),
                      hr(),
...
server<-function(input,output){
  ...
  output$statestable<-renderDataTable(read_excel(paste("www/",input$selinput,".xlsx",sep = "")),
                                      options = list(pageLength=10),escape = c('State Code'))
  output$path<-reactive(input$selinput1)
